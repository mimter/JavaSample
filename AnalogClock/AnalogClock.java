...
public class AnalogClock extends Applet implements Runnable {
	...
	void DispTime( ) {
		...
		// 時間の針描画
        MakeHand(CenterX,CenterY,hour * 30 + minute /60.0,
                 Radius * 0.8,1.2,Color.black);
		...
		// 分の針描画
        MakeHand(CenterX,CenterY,minute * 6.0,Radius * 0.9,0.7,Color.black);
    ...
    void MakeHand(int cx ,int cy, double kakudo,double nagasa,
                    double hutosa,Color color)
        {
            int px[]=new int[4];
            int py[]=new int[4];
